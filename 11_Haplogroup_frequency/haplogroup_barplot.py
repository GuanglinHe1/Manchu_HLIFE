...
功能（按顺序）：
1. 读取群体单倍群频率矩阵
2. 合并元数据（从元数据表提取 `merge_key` 与 `group_col`）
...
def run(
    ...
    if merge_key not in df_meta.columns:
        raise KeyError(f"元数据缺少合并键列：{merge_key}")
    if group_col not in df_meta.columns:
        raise KeyError(f"元数据缺少分组列：{group_col}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="单倍群频率柱状图生成")
    parser.add_argument("--input", "-i", required=True, help="输入频率矩阵 CSV")
    parser.add_argument("--prepared", "-p", required=True, help="元数据 CSV（用于提供分组列）")
